import {nRandomIndices} from "@/lib/utils/utils";
 ...
 */
export default function selectionValidation(selection: any, rules: any[]): any[] {
 ...
 */
export function selectionCorrection(selection: any, rules: any[]): any[] {
    const maxAttempts = 20;

    // Find required selections
    const requiredSelections = rules.filter((el: any) => el?.select === undefined);

    // Remove required selections
    let choices = rules.filter((el: any) => !requiredSelections.includes(el));
    ...
    let matches: any[], remaining: any[], inds: number[], greedy: Set<any>, sel: any, tryAgain: boolean;
    let cnt = 0;
    do {
        // Reset
        cnt += 1;
        ...
        for (let choice of choices) {
            ...
            sel = sel.filter((el: any) => !greedy.has(el));
            matches = choice.options.filter((el: any) => sel.includes(el));
            ...
            if (matches.length < choice.select) {
                ...
                remaining = choice.options.filter((el: any) => !greedy.has(el)).filter((el: any) => !matches.includes(el));
            ...
            matches.slice(0, choice.select).forEach((el: any) => greedy.add(el));
        }
 ...
 */
export function selectionCheck(selection: any, rules: any[]): boolean {
    selection = Array.from(new Set(selection));

    // Check selection isn't missing required selections
    const missingRequiredSelections = rules.filter((el: any) => el?.select === undefined).filter((el: any) => !selection.includes(el));
    ...
    // Remove required selections
    let choices = rules.filter((el: any) => !selection.includes(el));
    let sel = selection.filter((el: any) => !rules.includes(el));

    // Check rules that offer choices
    let total = 0;
    let matches: Set<any> = new Set([]);
    for (let choice of choices) {
        total += choice.select;

        // Check that there aren't too few selections for this choice
        if (choice.select > choice.options.reduce((count: number, el: any) => count +  sel.includes(el), 0)) {
            console.log('Selection Validation - Too few selections');
            return false;
        }

        // Track selections that have matched
        choice.options.forEach((el: any) => sel.includes(el) ? matches.add(el) : el);
    }
